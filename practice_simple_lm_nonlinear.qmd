...
```{r}
scatter = ggplot(Boston, aes(medv, lstat)) +
  geom_point()
scatter
```
...
Poly grabs random data points to compare within the model
...
```{r}
# My own model
lm.fitty = lm(medv ~ lstat + I(lstat^2) + log(lstat^5), data = Boston)
summary(lm.fitty)
```
...
ggplot(Boston, aes(x = lstat, y = medv)) +
  geom_point() +
  geom_smooth(method = "lm", formula = y ~ x + I(x^2) + log(x, 5), color = "blue")
```
...
```{r}
lm.betterfit = lm(medv ~ lstat + I(lstat^10) + log(lstat^5), data = Boston)
summary(lm.betterfit)

anova(lm.betterfit, lm.fitty, lm.fit5, lm.quad)
```
