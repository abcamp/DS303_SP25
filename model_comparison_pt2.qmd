...
```{r}
Uniform <- lm(Size ~ Height * Weight, data = Student_stats)

Dualform <- lm(Size ~ Height + Weight + Height:Weight, data = Student_stats)

Triform <- lm(Size ~ Height + Height:Weight, data = Student_stats)

AIC(Uniform, Dualform, Triform)
```
...
```{r}
summary(m.best) # the model we're trying to beat! 
```
...
```{r}
m.challenger <- lm(mpg ~ wt + cyl:gear + disp:wt, data = mtcars)
summary(X)

AIC(m.best, m.challenger) # i cant beat it ! haha

step(X, direction = "both")
```
