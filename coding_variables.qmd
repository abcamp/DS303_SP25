...
2.  When is this coding scheme preferred? What types of data or analysis scenarios make this coding method particularly useful?\
    This coding scheme is preferred when you want to compare each level to an overall mean rather than to a specific category.

3.  What are the advantages of this coding method? How does it simplify interpretation or enhance model performance?

    Its doing the same process as dummy coding but it is more specific. It also works with Anova() well.
...
It specifies the output more. Dummy coding would output 0 or 1 but Simple coding, because it compares does 1/k, would output a number with more decimal places which could help us understand the data more.
...
2 asian, 3 african am, 4 caucasian\
\
The reference group is Hispanic which is being compared to the other 3 races individually. For every 1 increase in test scores, Asian scores go up 11.54, African American scores go up 1.74, and Caucasian scores go up 7.59. African American however was not a significant factor in the model. This is likely due to how little data there is that is also spread apart whereas Asian, which also has little amounts of data, is not spread apart. The intercept is the average writing score for Hispanic.\
