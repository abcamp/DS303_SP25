...
with(P106_clean, plotVowels(f1, f2, ipa, plot.tokens = TRUE, pch.tokens = ipa, cex.tokens = 1.2, alpha.tokens = 0.2, plot.means = TRUE, pch.means = ipa, cex.means = 2, var.col.by = ipa, ellipse.line = TRUE, pretty = TRUE))
...
-   mixed effects models (linear or logistic): include random effects to account to within-group similarities
...
m3 <- glm(sex ~ age, data = all_clean, family = binomial)
summary(m3)

m4 <- lm(f0 ~ age, data = all_clean)
summary(m4)

m5 <- lm(duration ~ home_state, data = all_clean)
summary(m5)
levels(all_clean$home_state)
```
